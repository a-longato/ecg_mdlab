...
def bin_array(data, window_size=5):
    
    sequence_length, num_channels = data.shape
    
    # Calculate the new sequence length after binning
    new_sequence_length = sequence_length // window_size
    
    # Reshape the array to group the data into bins

    reshaped = data[:new_sequence_length * window_size].reshape(new_sequence_length, window_size, num_channels)
    
    # Compute the mean along the window dimension (axis=1)
